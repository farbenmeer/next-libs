...
export class HttpClient {
  ...
  prepareBody(body: unknown, contentType?: "form" | "json" | "form-data") {
    ...
    if (contentType === "form-data")
      return typeof body === "object" && body instanceof FormData ? body : formData(body as never);
    return body as any;
  }
  ...
  *prepareHeaders(headers: Record<string, string | string[]>): Generator<[string, string]> {
    for (const [key, value] of Object.entries(headers))
      ...
      else for (const val of value) yield [key, val];
  }
  ...
  mergeUrl(
    { url }: RequestSpecification,
    { query = {}, baseUrl = this.options.baseUrl }: HttpClientOptions,
  ) {
    return buildUrl(url, baseUrl, query);
  }
}

function lowercaseKeys<T extends Record<string, unknown>>(
  obj: T,
): {
